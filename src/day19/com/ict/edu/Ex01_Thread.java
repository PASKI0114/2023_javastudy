...
//스레드 (일꾼): 프로세스 내에서 실행되는 세부 작업 단위
//멀티스레드 : 하나의 프로세스에서 여러개의 스레드가 병행적으로 처리되는 것
//싱글스레드 : 하나의 프로세스에 하나의 스레드가 일처리 하는 것(day01~day18)

//java에서 스레드 생성법
// ** 자바에서는 start()를 호출하여 run()가 실행되면 스레드 실행됐다고 함.
//	1.Thread 클래스 상속받기 : start(), run() 메서드가 모두있다.
...
public class Ex01_Thread {
	...
	public static void main(String[] args) {
		//지금은 싱글스레드이다.(즉, 하나의 스레드가 모든일 을 다 처리한다.)
		//Thread.currentThread().getName() : 현재 일하는 스레드 이름 출력
		//main 메서드에서 일하는 스레드를 main스레드라고 한다.
