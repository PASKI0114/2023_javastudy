	...
	public Ex05_Dialog() {
		...
		jb1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// 메세지 다이얼로그 : 사용자에게 정보를 알리는 역할만 한다.(js 자바스트립트에서는 alert())
				// JOptionPane.showMessageDialog(parentComponent, message);
				// JOptionPane.showMessageDialog(jp, "방가방가");
				// JOptionPane.showMessageDialog(getParent(), "방가방가");

				// JOptionPane.showMessageDialog(parentComponent, message, title, messageType);
				// messageType: ERROR_MESSEGGE, INFPORMATION(기본), WARNING_MASSAGE,
				// QUESTION_MASSAGE, PLAIN_MASSAGE(없음)
				JOptionPane.showMessageDialog(jp, "방가방가", "MASSAGE 창", JOptionPane.WARNING_MESSAGE);
		...
		jb2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// optionType: YES_NO CALCEL_OPTION(예,아니오,취소),YES_NO_OPTION(예,아니오,취소),
				// OK_CALCEL_OPTION(확인, 취소)
		...
		jb3.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 입력 다이얼 로그 : 사용자에게 정보를 받는 다이얼로그(js 자바스크립트에서는 prompt() == 거의 사용안함)
		...
		jb4.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 옵션다이얼로그 : 사용자 다이얼로그(사용자가 만드는 다이얼로그)
				// JOptionPane.showOptionDialog(parentComponent,
				// message,title,optionType,messageType, null,null,null);
