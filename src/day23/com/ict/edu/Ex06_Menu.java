...
public class Ex06_Menu extends JFrame{
	...
	public Ex06_Menu() {
		...
		i_newFile.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(size == 0) {
					...
					int res = JOptionPane.showOptionDialog(getParent(), "변경된", "간단 메모장",
		...
		i_saveFile.addActionListener(new ActionListener() {	
			...
			public void actionPerformed(ActionEvent e) {
				// 파일 저장 다이얼 로그
				FileDialog fd = new FileDialog((JFrame)getParent(),"불러오기",FileDialog.SAVE);
				fd.setVisible(true);
				//실제 저장 코딩(I/O)
			}
		});
		...
		i_help.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				//메세지창 호출
				JOptionPane.showMessageDialog(getParent(), "간단메모장정보");
			}
		});
